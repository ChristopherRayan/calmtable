...
export default function ManagerDashboard() {
  ...
  if (!analytics) return <div className="text-gray-400 dark:text-white/40">Loading analytics...</div>;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <motion.div
            ...
            className="p-6 rounded-2xl bg-white dark:bg-[#140d09] border border-gray-200 dark:border-white/5 shadow-xl"
          >
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest">{stat.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Top Dishes */}
        <div className="p-8 rounded-3xl bg-white dark:bg-[#140d09] border border-gray-200 dark:border-white/5 shadow-xl">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-widest mb-6">Popular Menu Items</h3>
          <div className="space-y-4">
            {analytics.top_dishes.map((dish, idx) => (
              <div key={dish.menu_item_id} className="flex items-center justify-between group">
                <div className="flex items-center space-x-4">
                  <span className="text-xs font-bold text-gray-300 dark:text-white/20">0{idx + 1}</span>
                  <span className="text-sm text-gray-700 dark:text-white/80 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{dish.name}</span>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[10px] font-bold text-gray-400 dark:text-white/40">{dish.quantity} sold</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-8 rounded-3xl bg-white dark:bg-[#140d09] border border-gray-200 dark:border-white/5 shadow-xl">
          <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-widest mb-6">Reservation Volume</h3>
          <div className="h-48 flex items-end justify-between px-2 gap-1">
            {analytics.reservation_volume.slice(-14).map((pt, i) => (
              <div key={pt.date} className="flex-1 flex flex-col items-center group">
                ...
                />
                <div className="mt-2 text-[8px] text-gray-400 dark:text-white/20 uppercase tracking-tighter transform -rotate-45">{new Date(pt.date).toLocaleDateString(undefined, { weekday: 'short' })}</div>
              </div>
            ))}
          </div>
        </div>
